...
function DaftarAkun() {
  ...
  return (
    <DaftarMasukLayout 
      title="Daftar Akun" 
      subtitle="Mulai petualangan Anda di Ampatheia"
    >
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
        ...
        <button type="button" onClick={() => navigate("/")} className={demoButtonStyle}>
          🚀 Lewati & Masuk Beranda (Demo Mode)
        </button>
      </form>

      <p className="text-sm text-center text-slate-600 mt-8 font-medium">
        Sudah punya akun? <Link to="/masuk" className={`${orangeTextFigma} font-bold hover:underline`}>Masuk</Link>
      </p>
    </DaftarMasukLayout>
  );
}
